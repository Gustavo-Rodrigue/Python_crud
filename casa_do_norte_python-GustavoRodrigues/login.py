...
# Esta função é responsável por desenhar e controlar toda a tela de login. # Ela recebe 'app' como argumento, que é a janela principal da nossa  aplicação.
...
def showlogin(app):
    ...
    def attempt_login():
        username = userent.get().strip()  # Obtém o nome de usuário
        password = pwdent.get().strip()   # Obtém a senha

        # Verifica se ambos os campos foram preenchidos
        if not username or not password:
            messagebox.showwarning("Falha", "Preencha usuário e senha.")
            return  # <-- Agora retorna apenas se faltar algum campo
